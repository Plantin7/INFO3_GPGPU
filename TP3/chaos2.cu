...
__global__ void build_lookup() {
    __shared__ uint32_t lookup[1024]; // in the same block !
    uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x;
	// Build the table
    if(tid < 1024){
        lookup[tid] = 0;
        for(uint32_t i = 0; i < tid * tid; i++) 
        { 
        	lookup[tid] += 1;
        }
        __syncthreads (); 			// wait all threads in the same blocks

        // Check the table, there can be no zero entries
        if(tid < 32){
            for(uint32_t i = 0; i < 1024; i+=32)
            {
                if(lookup[tid+i] != (tid+i) * (tid+i))
                {
                	printf("Error on entry %u!\n", tid+i);
            	}
            }
        }
    }
